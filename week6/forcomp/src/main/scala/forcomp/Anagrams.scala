...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.toLowerCase().groupBy(_.toChar) map (x => (x._1, x._2.length))).toList sortWith (_._1 < _._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    for {
      occurrence <- x
      if !(y exists (z => occurrence._1 == z._1 && occurrence._2 == z._2))
    } yield (occurrence._1, occurrence._2 - y.find(_._1 == occurrence._1).getOrElse((' ', 0))._2)
  }
    ...
    def loop(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty)
        ...
        for {
          ...
          sentenceAnagram <- loop(subtract(occurrences, combination))
        } yield word :: sentenceAnagram
      }
    }
