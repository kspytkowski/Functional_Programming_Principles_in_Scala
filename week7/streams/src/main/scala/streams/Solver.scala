...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = Stream.cons((b.left, Left :: history), Stream.cons((b.right, Right :: history), Stream.cons((b.up, Up :: history), Stream.cons((b.down, Down :: history), Stream.empty)))) filter (x => x._1.isLegal)
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case x #:: xs => x #:: from(xs #::: newNeighborsOnly(neighborsWithHistory(x._1, x._2), explored), explored + x._1)
  }
